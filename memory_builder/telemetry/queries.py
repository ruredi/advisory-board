...
def get_run_progress(store: SQLiteStore, persona_id: str, run_id: int) -> RunProgress | None:
    ...
    return RunProgress(
        ...
        sources_processed=int(run["sources_processed"] or 0),
...
def list_sync_runs(
    ...
    *,
    ...
    rows = store.connect().execute(
        """
        SELECT id, persona_id, started_at, finished_at, sources_discovered, sources_processed,
    ...
    for row in rows:
        ...
        if float(item["cost_usd"] or 0) == 0 and live_cost.cost_usd > 0:
            item["cost_usd_display"] = live_cost.cost_usd
        else:
            item["cost_usd_display"] = float(item["cost_usd"] or 0)
